...
func (s *Store) CreateInvoice(inv *Invoice) (int, error) {
	...
	res, err := tx.Exec(`
		INSERT INTO invoices (invoice_number, date, sender_name, sender_address, recipient_name, recipient_address, tax_rate, status, is_small_business, customer_id, internal_note, document_note, payment_details)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
	`, inv.InvoiceNumber, inv.Date, inv.SenderName, inv.SenderAddress, inv.RecipientName, inv.RecipientAddress, inv.TaxRate, inv.Status, inv.IsSmallBusiness, inv.CustomerID, inv.InternalNote, inv.DocumentNote, inv.PaymentDetails)
	if err != nil {
...
func (s *Store) UpdateInvoice(inv *Invoice) error {
	...
	_, err = tx.Exec(`
		UPDATE invoices 
		SET invoice_number = ?, date = ?, sender_name = ?, sender_address = ?, recipient_name = ?, recipient_address = ?, tax_rate = ?, status = ?, is_small_business = ?, customer_id = ?, internal_note = ?, document_note = ?, payment_details = ?
		WHERE id = ?
	`, inv.InvoiceNumber, inv.Date, inv.SenderName, inv.SenderAddress, inv.RecipientName, inv.RecipientAddress, inv.TaxRate, inv.Status, inv.IsSmallBusiness, inv.CustomerID, inv.InternalNote, inv.DocumentNote, inv.PaymentDetails, inv.ID)
	if err != nil {
...
func (s *Store) GetInvoice(id int) (*Invoice, error) {
	...
	var i Invoice
	err := s.DB.QueryRow(`
		SELECT i.id, i.invoice_number, i.date, i.sender_name, i.sender_address, i.recipient_name, i.recipient_address, i.tax_rate, i.created_at, i.status, i.is_small_business, i.customer_id, COALESCE(c.customer_number, ''), COALESCE(i.internal_note, ''), COALESCE(i.document_note, ''), COALESCE(i.payment_details, '')
		FROM invoices i
		LEFT JOIN customers c ON i.customer_id = c.id
		WHERE i.id = ?
	`, id).Scan(&i.ID, &i.InvoiceNumber, &i.Date, &i.SenderName, &i.SenderAddress, &i.RecipientName, &i.RecipientAddress, &i.TaxRate, &i.CreatedAt, &i.Status, &i.IsSmallBusiness, &i.CustomerID, &i.CustomerNumber, &i.InternalNote, &i.DocumentNote, &i.PaymentDetails)
	if err != nil {
